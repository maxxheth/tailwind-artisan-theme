...
function addTypographyClasses() {
	const editorLoadedInterval = setInterval(() => {
		...
		if (
			...
			if (getCurrentPostTypeClass()) {
				...
				for (const className of Object.values(targetClasses)) {
					className.push(getCurrentPostTypeClass());
				}
			}

			// Add the classes before creating the mutation observer.
			for (const [targetClass, classes] of Object.entries(targetClasses)) {
				document
			...
			for (const className of Object.keys(targetClasses)) {
